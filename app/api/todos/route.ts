...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "할 일을 불러오지 못했습니다." }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "할 일을 저장하지 못했습니다." }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!completedOnly) {
      return NextResponse.json({ error: "삭제 조건이 필요합니다." }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "완료한 할 일을 지우지 못했습니다." }, { status: 500 });
  }
}
